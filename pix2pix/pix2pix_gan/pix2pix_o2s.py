...
DIR = '/common/home/jp1891/Downloads/CS536_ML/Task2/data/combine/train/'
DIR_VAL = '/common/home/jp1891/Downloads/CS536_ML/Task2/data/combine/val/'
n_gpus = 4
batch_size = 6
...
for i in tqdm(range(epoch, EPOCHS)): 
    ...
    #applying model on validation set to generate images.
    j =0
    for (inputs, targets), _ in tqdm(val_dl,leave=False):
        inputs = inputs.to(device)
        targets = targets.to(device)
        generated_output =  generator(inputs)
        super_img = [inputs[:10], targets[:10], generated_output[:10]]
        super_img_cat = torch.cat(super_img, 0) #concat input, target and generated output images.
        vutils.save_image(generated_output.data[:30], "/common/home/jp1891/Downloads/CS536_ML/Task2/generated_images/"+'sample_3_%d'%(i+1) + '.png', nrow=5, normalize=True)
        vutils.save_image(super_img_cat, "/common/home/jp1891/Downloads/CS536_ML/Task2/generated_images/grids/"+'sample_3_%d_%d'%(j, (i+1)) + '.png', nrow=5, normalize=True)
        j += 1
    
    # storing losses as CSV
